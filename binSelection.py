...
def selection(bin_set, limit):
    new_list = []
    for num_bin in bin_set:
        if num_bin < limit:
            new_list.append(num_bin)
    return new_list


def calculateBinStatic(x1, x2, method=2):
    x1BinSet = computeBinSetStatic(x1)
    x2BinSet = computeBinSetStatic(x2)
    value, bin_result = computeValue(x1, x2, x1BinSet, x2BinSet, method)
    return bin_result[0], bin_result[1]
    ...
    for numbinx in xiBinSet:
        for numbiny in yBinSet:
            if (method == 0):
                ...
                binValue = round(cm.ud_ixy(xi, y, int(numbinx), int(numbiny)), 2)
            elif (method == 5):
                binValue = round(cm.cd_ixy(xi, y, int(numbinx), int(numbiny)), 2)
...
def computeBinSetStatic(f):
    binSet = []
    N = len(f)
    domainSize = float(len(set(f)))  # contains the unique elements in f
    std = np.std(f)
...
def cuadratureSearchBins(X, method=1, split=3, xjlist=False, consecutiveDepth=3, maxDepth=7, computeRepeated=False,
                         Debug=False):
    xbinsetList = {}
    xValueList = {}
